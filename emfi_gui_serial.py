...
class EMFIScannerGUI:
    ...
    def create_visualization_section(self, parent):
        ...
        self.viz_notebook.add(monitor_frame, text="Target Serial Monitor")
    ...
    def serial_monitor_loop(self):
        """Continuously read from target serial and display"""
        while self.monitor_running:
            try:
                if self.controller.target_connected and self.controller.target_ser:
                    if self.controller.target_ser.in_waiting > 0:
                        data = self.controller.target_ser.read(
                            self.controller.target_ser.in_waiting
                        ).decode('utf-8', errors='replace')
                        
                        if data:
                            self.root.after(0, self.append_to_monitor, data, "data")
    ...
    def append_to_monitor(self, text, msg_type="data"):
        """Append text to serial monitor with optional formatting"""
        try:
            self.serial_monitor.config(state=tk.NORMAL)
            
            if self.timestamp_var.get() and msg_type == "data":
                timestamp = time.strftime("[%H:%M:%S] ")
                self.serial_monitor.insert(tk.END, timestamp, "timestamp")
            
            # Apply color tags based on message type
            if msg_type == "system":
                self.serial_monitor.insert(tk.END, text, "system")
            elif msg_type == "error":
                self.serial_monitor.insert(tk.END, text, "error")
            else:
